...
// Define API base URL
const API_URL = process.env.REACT_APP_API_URL || 'https://backend-yytm.onrender.com';
...
async function loadOverlayLayers() {
    ...
    try {
        const localResponse = await fetch(`${API_URL}/api/local`);
        ...
        const districtResponse = await fetch(`${API_URL}/api/district`);
        ...
        const provinceResponse = await fetch(`${API_URL}/api/province`);
...
async function loadRoadBufferData() {
    try {
        ...
        const roadResponse = await fetch(`${API_URL}/api/roads`);
        ...
        const houseResponse = await fetch(`${API_URL}/api/buildings`);
        ...
        const hospitalResponse = await fetch(`${API_URL}/api/hospitaa`);
